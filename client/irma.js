...
var loglevel = Loglevel.Info; // Determines which messages are written to log
...
function verify(request, success_cb, cancel_cb, failure_cb) {
    ...
    if (typeof request === "object") {
        log(Loglevel.Warn, "calling IRMA.verify with a bare service provider request "
...
function showPopup() {
    if (ua === UserAgent.Desktop) {
        ...
        + "<div class='irma-title' id='irma-title'></div>"
        + "<p id='irma-text'></p>"
        ...
        + "<button class='irma-button' id='irma-cancel-button'></button>"
        + "</div>"
        + "</div>"
        + "</div></div></div></div>")
            .appendTo("body");
